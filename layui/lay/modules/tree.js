...
layui.define("jquery", function(e) {
            ...
            if (subTreeNodes && subTreeNodes.length > 0) {
                for (var ind = 0; ind < subTreeNodes.length; ind++) {
                    ...
                    if (subTrNode) {
                        ...
                        !isOpened ? (e.data("spread", null), subTrNode.setAttribute('class', 'layui-hide') /*.addClass("layui-hide"), subTrNode.removeClass("layui-show"),*/ /*r.html(t.arrow[0]), n.find(".layui-icon").html(t.branch[0])*/ ) :
                            (e.data("spread", !0), subTrNode.setAttribute('class', '') /*.removeClass("layui-hide"), subTrNode.addClass("layui-show"),*/ /*r.html(t.arrow[1]), n.find(".layui-icon").html(t.branch[1])*/ )
                    }
    ...
    i.prototype.traverseModel = function(treeTable, parentNode, item, childrenAttrs) {
        ...
        i.prototype.initGird = function(e) {
            ...
            tt = new TreeTable();
            // var jsonStore = {children:o.options.nodes};
            ...
            e.addClass("layui-tree"),
                ...
                /*e.append('<colgroup><col width="200"></colgroup>'),*/
        ...
        i.prototype.treeGird = function(e, a) { //生成树
            var r = this,
                i = r.options,
                n = a || i.nodes;
            ...
            layui.each(n, function(a, n) {
                ...
                var p = treeNode.parentId == 'root' ? null : treeNode.parentId,
                    ...
                    /*                          str = '<tr class="' + (p ? "layui-hide" : "") + '" id="'+n.id+'" level="'+n.level+'">'
                                                     + '<td style="width:5%">'+index+'</td>'
                                                     + '<td><li '+ (n.spread ? 'data-spread="' + n.spread + '"' : "") + '>'
                                                     + indent +(l ? '<i class="layui-icon layui-tree-spread">' + (n.spread ? t.arrow[1] : t.arrow[0]) + "</i>" : "")
                                                     + '<a href="' + (n.href || "javascript:;") + '" ' + (i.target && n.href ? 'target="' + i.target + '"' : "") + ">" + ('<i class="layui-icon layui-tree-' + (l ? "branch" : "leaf") + '">' + (l ? n.spread ? t.branch[1] : t.branch[0] : t.leaf) + "</i>") + ("<cite>" + (n.name || "未命名") + "</cite></a></li></td>")
                                                     + '<td>1</td>'
                                                     + "</tr>";*/
                    ...
                    str = o(['<tr class="' + (p ? "layui-hide" : "") + '" id="' + n.id + '">',
                        ...
                        /*function() {
                            return  '<td><li '+ (n.spread ? 'data-spread="' + n.spread + '"' : "") + '>';
                        }(),
                        function() {
                            return  indent +(l ? '<i class="layui-icon layui-tree-spread">' + (n.spread ? t.arrow[1] : t.arrow[0]) + "</i>" : "");
                        }(),
                        function() {
                            return '<a href="' + (n.href || "javascript:;") + '" ' + (i.target && n.href ? 'target="' + i.target + '"' : "") + ">" + ('<i class="layui-icon layui-tree-' + (l ? "branch" : "leaf") + '">' + (l ? n.spread ? t.branch[1] : t.branch[0] : t.leaf) + "</i>") + ("<cite>" + (n.name || "未命名") + "</cite></a></li></td>")
                        }(), */
                        function() {
                            var ret = ""
                            for (var ind = 0; ind < i.layout.length; ind++) {
                                /*var field = i.layout[ind].field;
                                var value = n[i.layout[ind].field];*/
                                /*ret += '<td class="'+i.layout[ind].className+'" style="'+i.layout[ind].style+'">'+n[i.layout[ind].field]+'</td>',*/
                ...
                e.append(str),
                    ...
                    // "function" == typeof i.click && r.click(str, n),
                    r.spreadGird(str, n),
                    i.drag && r.drag(str, n)
                r.changed(str, n)
            })
        ...
        i.prototype.cancelParentsCheckboxCheck = function(ele) {
            ...
            if (!childrendInputs || childrendInputs.length == 0) {
                ...
                for (var j = 0, len2 = childrendInputs.length; j < len2; j++) {
                    //console.log(childrendInputs[j])
                    //console.log(childrendInputs[j].getAttribute("data-parent-id") + "====" + childrendInputs[j].checked)
                    //console.log("--------------------------------------------------------")
        ...
        i.prototype.spreadGird = function(e, o) {
            var a = this,
                ...
                nodeId = e[0].id,
                /*bo = e.nextAll("[parent-node='"+nodeId+"']").find("[parent-node='"+nodeId+"']"),
                i = e.children("ul"),*/
                // n = e.find("a"),
